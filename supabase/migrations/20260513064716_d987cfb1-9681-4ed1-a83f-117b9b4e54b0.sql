-- ============================================================================
-- PRECISION 12 — Avaliação Fabrik / Foundation (E1) · IDEMPOTENT REWRITE v3
-- ============================================================================
-- Reapply via tool (file already in repo). Includes Section 3.5.1 backfill.
-- ============================================================================
...
create policy "subjective_scores_trainer_own_or_admin"
  on public.subjective_scores for all to authenticated
  using (
...
create policy "student_external_professionals_trainer_own_or_admin"
  on public.student_external_professionals for all to authenticated
  using (
    exists (select 1 from public.students s
      where s.id = student_id
        and (s.trainer_id = auth.uid()
...
create policy "precision_reports_trainer_own_or_admin"
  on public.precision_reports for all to authenticated
  using (
    exists (select 1 from public.students s
      where s.id = student_id
        and (s.trainer_id = auth.uid()
...
create policy "dexa_pdfs_trainer_own_or_admin_select"
  ...
  using (
    ...
    and exists (select 1 from public.students s
      where s.trainer_id = auth.uid() and storage.objects.name like s.id::text || '/%')
  );
