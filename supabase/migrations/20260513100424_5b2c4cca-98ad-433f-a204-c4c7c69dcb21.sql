...
BEGIN
  ...
  IF v_child_kind = 'vo2' THEN
    INSERT INTO public.vo2_assessment_details (
      assessment_id, fc_max_predicted, fc_peak, vo2_final, vo2_classification,
      recovery_drop_1min, recovery_classification, total_time_min,
      final_speed_kmh, final_incline_pct, protocol_name, last_valid_load,
      last_valid_watts, abort_reason
    )
    SELECT v_assessment.id, x.fc_max_predicted, x.fc_peak, x.vo2_final,
           x.vo2_classification, x.recovery_drop_1min, x.recovery_classification,
           x.total_time_min, x.final_speed_kmh, x.final_incline_pct,
           x.protocol_name, x.last_valid_load, x.last_valid_watts, x.abort_reason
    FROM jsonb_to_record(COALESCE(p_child_data, '{}'::jsonb)) AS x(
      fc_max_predicted int, fc_peak int, vo2_final numeric,
      vo2_classification text, recovery_drop_1min int,
      recovery_classification text, total_time_min numeric,
      final_speed_kmh numeric, final_incline_pct numeric, protocol_name text,
      last_valid_load numeric, last_valid_watts int, abort_reason text
    );
    ...
    IF p_bike_stages IS NOT NULL
      ...
      INSERT INTO public.vo2_bike_stages (
        assessment_id, stage_order, time_label, phase, load_value,
        rpm_target, watts_observed, hr_final, pse, vo2_estimated, notes
      )
      SELECT v_assessment.id, s.stage_order, s.time_label, s.phase,
             s.load_value, s.rpm_target, s.watts_observed, s.hr_final,
             s.pse, s.vo2_estimated, s.notes
      FROM jsonb_to_recordset(p_bike_stages) AS s(
        stage_order int, time_label text, phase text, load_value numeric,
        rpm_target text, watts_observed int, hr_final int, pse int,
        vo2_estimated numeric, notes text
      );
    END IF;
  ELSIF v_child_kind = 'handgrip' THEN
    INSERT INTO public.handgrip_results (
      assessment_id, dominant_hand, right_kg_attempts, left_kg_attempts,
      right_kg, left_kg, classification
    )
    VALUES (
      v_assessment.id,
  ...
  ELSIF v_child_kind = 'dexa' THEN
    INSERT INTO public.dexa_results (
      assessment_id, total_mass_kg, fat_mass_kg, fat_pct, lean_mass_kg,
      bone_mass_kg, bone_density_z_score, visceral_fat_g, android_gynoid_ratio,
      scan_pdf_url, bmr_harris_benedict_kcal, bmr_mifflin_stjeor_kcal,
      appendicular_lean_mass_kg, imma_baumgartner, fmi, fat_percentile,
      regional_distribution, conclusion_text, scan_pdf_storage_path,
      raw_extracted_json, extraction_confidence, extraction_method
    )
    SELECT v_assessment.id, x.total_mass_kg, x.fat_mass_kg, x.fat_pct,
           x.lean_mass_kg, x.bone_mass_kg, x.bone_density_z_score,
           x.visceral_fat_g, x.android_gynoid_ratio, x.scan_pdf_url,
           x.bmr_harris_benedict_kcal, x.bmr_mifflin_stjeor_kcal,
           x.appendicular_lean_mass_kg, x.imma_baumgartner, x.fmi,
           x.fat_percentile, x.regional_distribution, x.conclusion_text,
           x.scan_pdf_storage_path, x.raw_extracted_json,
           x.extraction_confidence, x.extraction_method
    FROM jsonb_to_record(COALESCE(p_child_data, '{}'::jsonb)) AS x(
      total_mass_kg numeric, fat_mass_kg numeric, fat_pct numeric,
      lean_mass_kg numeric, bone_mass_kg numeric, bone_density_z_score numeric,
      visceral_fat_g numeric, android_gynoid_ratio numeric, scan_pdf_url text,
      bmr_harris_benedict_kcal int, bmr_mifflin_stjeor_kcal int,
      appendicular_lean_mass_kg numeric, imma_baumgartner numeric,
      fmi numeric, fat_percentile int, regional_distribution jsonb,
      conclusion_text text, scan_pdf_storage_path text,
      raw_extracted_json jsonb, extraction_confidence numeric,
      extraction_method text
    );
  ELSIF v_child_kind = 'sit_to_stand' THEN
    INSERT INTO public.sit_to_stand_results (
      assessment_id, sit_score, sit_supports, sit_instabilities,
      rise_score, rise_supports, rise_instabilities, classification, notes
    )
    SELECT v_assessment.id, x.sit_score,
           COALESCE(x.sit_supports, '{}'::jsonb),
           COALESCE(x.sit_instabilities, 0),
           x.rise_score,
           COALESCE(x.rise_supports, '{}'::jsonb),
           COALESCE(x.rise_instabilities, 0),
           x.classification, x.notes
    FROM jsonb_to_record(COALESCE(p_child_data, '{}'::jsonb)) AS x(
      sit_score numeric, sit_supports jsonb, sit_instabilities int,
      rise_score numeric, rise_supports jsonb, rise_instabilities int,
  ...
  IF p_cardiovascular IS NOT NULL AND jsonb_typeof(p_cardiovascular) = 'object' THEN
    INSERT INTO public.cardiovascular_baseline (
      assessment_id, systolic_mmhg, diastolic_mmhg, resting_hr_bpm,
      on_medication, medication_details, reference_doctor_name,
      reference_doctor_contact, classification
    )
    SELECT v_assessment.id, x.systolic_mmhg, x.diastolic_mmhg, x.resting_hr_bpm,
           COALESCE(x.on_medication, false), x.medication_details,
           x.reference_doctor_name, x.reference_doctor_contact, x.classification
    FROM jsonb_to_record(p_cardiovascular) AS x(
      systolic_mmhg int, diastolic_mmhg int, resting_hr_bpm int,
      on_medication boolean, medication_details text,
      reference_doctor_name text, reference_doctor_contact text,
      classification text
    );
  END IF;
  ...
  IF p_subjective IS NOT NULL AND jsonb_typeof(p_subjective) = 'object' THEN
    INSERT INTO public.subjective_scores (
      student_id, assessment_id, recorded_at, sleep_score, energy_score,
      stress_score, recovery_score, wellbeing_score, mood_score, notes
    )
    SELECT v_student_id, v_assessment.id,
           COALESCE(x.recorded_at, v_assessment.assessment_date),
           x.sleep_score, x.energy_score, x.stress_score, x.recovery_score,
           x.wellbeing_score, x.mood_score, x.notes
    FROM jsonb_to_record(p_subjective) AS x(
      recorded_at date, sleep_score smallint, energy_score smallint,
      stress_score smallint, recovery_score smallint,
      wellbeing_score smallint, mood_score smallint, notes text
    );
  END IF;
